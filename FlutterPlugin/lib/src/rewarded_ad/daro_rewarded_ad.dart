...
import '../models/daro_reward_item.dart';
import '../internal/ad_event_data.dart';

typedef OnEarnedReward = void Function(DaroAdInfo adInfo, DaroRewardItem reward);

class DaroRewardedAd {
  static const MethodChannel _methodChannel =
      MethodChannel('daro_flutter/rewarded');
  static const EventChannel _eventChannel =
      EventChannel('daro_flutter/rewarded_events');

  static final Map<String, DaroRewardedAd> _instances = {};
      ...
      (dynamic event) {
        ...
        switch (eventData.eventType) {
          ...
          case 'onAdDismiss':
            ...
            break;
          case 'onEarnedReward':
            _handleRewardEvent(eventData.event, instance);
            break;
        }
      },
      onError: (dynamic error) {
        print('DaroRewardedAd event channel error: $error');
      },
    );
  }
  ...
  static void _handleRewardEvent(
    Map<String, dynamic> event,
    DaroRewardedAd instance,
  ) {
    final rawAdInfo = event['adInfo'];
    final rawReward = event['reward'];

    if (rawAdInfo is Map && rawReward is Map) {
      final adInfo = DaroAdInfo.fromMap(Map<String, dynamic>.from(rawAdInfo));
      final reward = DaroRewardItem.fromMap(Map<String, dynamic>.from(rawReward));
      instance.onEarnedReward?.call(adInfo, reward);
    }
  }
